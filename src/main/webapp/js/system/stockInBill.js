...
//添加明细
    ...
    $("#editForm").submit(function () {
        ...
        $.each(tbody, function (index, item) {
            $(item).find("[tag=pid]").prop("name", "stockInBill.items[" + index + "].product.id");
            $(item).find("[tag=costPrice]").prop("name", "stockInBill.items[" + index + "].costPrice");
            $(item).find("[tag=number]").prop("name", "stockInBill.items[" + index + "].number");
            $(item).find("[tag=remark]").prop("name", "stockInBill.items[" + index + "].remark");
        })
    });
...
$(function () {
    ...
    $("input[name='stockInBill.vdate']").addClass("Wdate").click(function () {
        WdatePicker({
            maxDate:new Date()
        });
    })
